...
import { StyleSheet ,ActivityIndicator, TouchableWithoutFeedback, Image, Alert } from "react-native";
...
class CommentItem extends React.Component {
  ...
  deleteComment = async()=>{
    ...
    firebase.firestore().collection("comments").doc(postId).collection("userComments").doc(comment.commentId).delete()
    .then(()=>{
  ...
  getCurrentUsername(){
    this.firestoreUsersRef.doc(firebase.auth().currentUser.uid).get().then((document)=>{
  ...
  render() {
    ...
    return(
        ...
        <Block row  style={{flex:1, paddingBottom:1, paddingTop:1}}>
            <Block>
              <TouchableWithoutFeedback onPress={()=> navigation.navigate('userProfile', {userId: userId})}>
              <Text size={12} style={styles.cardUser}>  {comment.username}</Text>
              </TouchableWithoutFeedback>
            </Block>
            <TouchableOpacity onLongPress={this.deleteComment}>
            <Block fluid style={styles.comment}>
              <Text italic  size={10} color="#1c1c1c" >{comment.comment}</Text> 
            </Block>
            </TouchableOpacity>
          </Block>
