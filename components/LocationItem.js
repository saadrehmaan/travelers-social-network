...
class LocationItem extends React.Component {
    ...
    handleDetailResults = async ()=>{
        const { fetchDetails, place_id, updateLocation, structured_formatting, clearSearch } = this.props;
        ...
        const locObject = {locationName, coordinates};
        clearSearch();
  ...
  render() {
    return(
        ...
        <TouchableOpacity style={styles.searchItem} onPress={this.handleDetailResults}> 
            <Text style={{paddingLeft:10}}>{this.props.structured_formatting.main_text}</Text>
            {/* <Text>{el.}</Text> */}
        </TouchableOpacity>
