...
class CreatePlan extends Component {
    ...
    firestorePlansRef = firebase.firestore().collection("plans").doc(this.user.uid).collection('userPlans');
...
getPlacesUrl = (lat, long, radius, type)=> {
  ...
  getPlaces = ()=> {
    ...
    fetch(url)
      ...
      .then(res => {
        ...
        res.results.map((element, index) => {
          const marketObj = {};
          let photoUrl = `https://maps.googleapis.com/maps/api/place/photo?photoreference=${element.photos[0].photo_reference}&sensor=false&maxheight=${element.photos[0].height}&maxwidth=${element.photos[0].width}&key=${key}`
    ...
    addSpot =(place_id, name, location, startDateTime, endDateTime, images)=>{
        let spot = {place_id: place_id, name:name, location:location, startDateTime:startDateTime, endDateTime:endDateTime, photos: images};
        let allSpots = this.state.spots;
        allSpots.push(spot);

        this.setState({spots: allSpots});

        const placeIndex = this.state.places.findIndex(item => item.place_id === place_id);
        // console.log(item);
    ...
    removeSpot = (place_id)=>{
        ...
        const spotIndex = this.state.spots.findIndex(item => item.place_id == place_id);
      ...
      getCurrentDate = ()=>{
        ...
        getCreatorName = ()=>{
            firebase.firestore().collection("users").doc(this.user.uid).get().then((doc)=>{
    ...
    render() {
        return (
            ...
            <Block row left shadow  flex={1} style={{paddingTop: 10, paddingLeft: 5}} >
            ...
            <Block shadow flex={16} style={{backgroundColor: '#f5f5f5', borderRadius: 8, marginBottom: 2 , width:width*0.9}}>
            ...
            <Block shadow center flex={6} style={{marginTop: 2,borderRadius: 8, backgroundColor: 'white', paddingBottom: 15 , width:width*0.8}}>
                <Block shadow flex = {1} row style={{padding:5, backgroundColor: '#f5f5f5', borderRadius: 15, marginBottom: 5 , marginTop:5, width:width*0.7}}>
                    <Block left flex={1}>
                        <Text h7>From</Text>
                ...
                <Block flex={2} >
                <Button style={{marginTop:10}} round size="small"  color="#3BBDE3" shadowless onPress={()=>{
            ...
            {/* 2 free */}
            <Block flex={1}><Text h4>Select Spots to visit:</Text></Block>
            <Block flex={7}>
            <View style={{ height: 140, marginTop: 5 }}>
                                ...
                                >
                                    ...
                                    <Category 
                                        imageUri={place.photo==""?"https://media-exp1.licdn.com/dms/image/C560BAQHMnA03XDdf3w/company-logo_200_200/0?e=2159024400&v=beta&t=C7KMOtnrJwGrMXmgIk2u1B8a7VRfgxMwXng9cdP9kZk":place.photo}
                                        name={place.name}
                                        // friend={false}
                                    />
                                    </TouchableOpacity>
                ...
                <Block center flex={2}>
                <Button style={{marginTop:10}} round size="large"  color="#ff6347" shadowless onPress={this.savePlan} >Save Plan</Button>
                </Block>
