...
if(this.state.found){
    return(
        ...
        <Block >
          <TouchableWithoutFeedback onPress={()=> navigation.navigate('userProfile',{userId: this.state.foundUser})}>
          <Text size={25} style={styles.cardUser}>{this.state.searchWord}</Text>
          </TouchableWithoutFeedback>
        </Block>
        
        </Block>
        <Block right flex={1} styles={{marginTop:30, marginRight:50}}>
        <Button onlyIcon icon="plus" iconFamily="antdesign" iconSize={30} color="tomato" iconColor="#fff" style={{ width: 50, height: 50 }}  onPress={this.addMember}></Button>
        </Block>
